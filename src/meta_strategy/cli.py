...
def backtest(
    strategy_name: str = typer.Argument(
        ..., help="Strategy name: bollinger-bands, supertrend, bull-market-support-band"
    ),
...
def backtest_all(
    ...
    start: str = typer.Option("2018-01-01", help="Backtest start date"),
    end: str | None = typer.Option(None, help="Backtest end date"),
    cash: float = typer.Option(100_000.0, help="Initial capital"),
    ...
    header = (
        f"{'Strategy':<28} {'Return%':>10} {'B&H%':>10} "
        f"{'WinRate%':>10} {'Trades':>8} {'MaxDD%':>10} {'Sharpe':>8} {'Equity$':>12}"
    )
    sep = "-" * len(header)
    typer.echo(header)
    typer.echo(sep)
    for r in results:
        line = (
            ...
            f"{r['sharpe_ratio']:>8.2f} {r['final_equity']:>12.2f}"
        )
        typer.echo(line)
    typer.echo(sep)
    ...
    if save:
        ...
        typer.echo("📄 Results saved to strategies/output/backtest-results.md")
...
def _save_results_markdown(results: list[dict], symbol: str) -> None:
    ...
    for r in results:
        line = (
            f"| {r['strategy']} | {r['return_pct']:.2f} | {r['buy_hold_return_pct']:.2f} | "
            f"{r['win_rate_pct']:.2f} | {r['num_trades']} | {r['max_drawdown_pct']:.2f} | "
            f"{r['sharpe_ratio']:.2f} | ${r['final_equity']:.2f} |"
        )
        lines.append(line)
...
def multi_asset_cmd(
    ...
    for r in results:
        if "error" in r:
            ...
            line = (
                f"{r['symbol']:<12} {r['return_pct']:>10.2f} {r['buy_hold_return_pct']:>10.2f} "
                f"{r['win_rate_pct']:>10.2f} {r['num_trades']:>8d} "
...
def optimize(
    ...
    if has_split:
        header = (
            f"{'Rank':<6} {'Params':<35} {'IS Ret%':>9} {'IS Sharpe':>10} "
            f"{'OOS Ret%':>9} {'OOS Sharpe':>11} {'Trades':>8} {'MaxDD%':>8} {'WinR%':>8}"
        )
        sep = "-" * len(header)
        typer.echo(header)
        typer.echo(sep)
        for i, r in enumerate(results[:top], 1):
            ...
            line = (
                f"{i:<6} {params_str:<35} {r['is_return_pct']:>9.2f} {r['is_sharpe_ratio']:>10.2f} "
                f"{r['return_pct']:>9.2f} {r['sharpe_ratio']:>11.2f} {r['num_trades']:>8d} "
                f"{r['max_drawdown_pct']:>8.2f} {r['win_rate_pct']:>8.2f}"
            )
            typer.echo(line)
    else:
        header = (
        ...
        for i, r in enumerate(results[:top], 1):
            params_str = ", ".join(f"{k}={v}" for k, v in r["params"].items())
            line = (
                f"{i:<6} {params_str:<40} {r['return_pct']:>10.2f} {r['sharpe_ratio']:>8.2f} "
                f"{r['num_trades']:>8d} {r['max_drawdown_pct']:>10.2f} {r['win_rate_pct']:>10.2f}"
            )
            typer.echo(line)
    typer.echo(sep)
    ...
    if results:
        ...
        if has_split:
            best_line = (
                ...
                f"(OOS Sharpe: {best['sharpe_ratio']:.2f}, IS Sharpe: {best['is_sharpe_ratio']:.2f})"
            )
            ...
            if warning:
                ...
                warning_msg = (
                    f"\n⚠️  OVERFITTING WARNING: In-sample Sharpe ({warning['is_sharpe']:.2f}) "
                    f"is {ratio_str} out-of-sample Sharpe ({warning['oos_sharpe']:.2f}). "
            ...
            best_line = (
                f"\n🏆 Best params: {best['params']} "
                f"(Sharpe: {best['sharpe_ratio']:.2f}, Return: {best['return_pct']:.2f}%)"
            )
            typer.echo(best_line)
...
def walk_forward_cmd(
    ...
    for f in result["folds"]:
        ...
        fold_line = (
            f"          train Sharpe: {f['train_sharpe']:.2f} → test Return: {f['test_return_pct']:.2f}%, "
            f"Sharpe: {f['test_sharpe']:.2f}, Trades: {f['test_trades']}, MaxDD: {f['test_max_dd_pct']:.2f}%"
        )
    ...
    if stability and stability.get("params_per_fold"):
        ...
        if stability["changes"]:
            ...
            for c in stability["changes"]:
                change_line = (
                    f"      {c['param']}: fold {c['fold_from']}→{c['fold_to']}: "
                    f"{c['prev']} → {c['curr']} ({c['pct_change']:.0f}% change)"
                )
                typer.echo(change_line)
        else:
...
def export(
    ...
    from .backtest import run_all_backtests
    from .reports import export_results_csv, export_results_json
